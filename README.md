...
| レベル | テーマ | 主な内容 |
| --- | --- | --- |
| 🌱 初級編 (Lv.1) | プログラミングの第一歩 | Webの全体像、Java/JS/SQLの基礎文法 |
| 🔧 中級編 (Lv.2) | 動くものを作れるようになる | 配列操作・DOM・非同期通信、SQL集計/JOIN、Javaクラス/例外 |
| 🚀 上級編 (Lv.3) | 設計と仕組みを理解する | クロージャ/OOP、サブクエリ/インデックス、Stream、3層設計、セキュリティ基礎 |
| 👑 神レベル (Lv.4) | システム全体を統べる | SOLID設計、システム設計、セキュリティ、パフォーマンス、テスト/CI/CD、総合演習 |
...
| ✨ JavaScript | 変数・制御・関数・配列/オブジェクト・DOM・非同期/API・クロージャ・OOP |
| 🗄️ SQL | SELECT〜JOIN・集計・更新/トランザクション・サブクエリ・インデックス |
| ☕ Java | 型・制御・クラス・コレクション/例外・Spring・ジェネリクス/Stream |
| 🧩 総合演習 | 1機能を端から端まで・データ設計・セキュリティ |
| 👑 神レベル | 設計原則・システム設計・セキュリティ・性能・テスト/CI・最終課題 |
